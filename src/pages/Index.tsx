...
import { Cable, ChevronRight, Shield, Zap, Award, UserCog, ShoppingCart, Quote, X } from "lucide-react";
...
type AuthMode = "none" | "login" | "signup" | "otp" | "password-reset";

const Index = () => {
  const navigate = useNavigate();
  const { isAuthenticated: isOwnerAuth } = useOwnerAuth();
  const { isAuthenticated: isUserAuth, pendingOtp } = useUserAuth();
  const [authMode, setAuthMode] = useState<AuthMode>("none");

  const handleAuthClose = () => {
    if (authMode === "otp" && pendingOtp) {
      return;
    }
    setAuthMode("none");
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-background via-accent/20 to-background">
        ...
        <div className="container mx-auto px-4 py-20 md:py-32">
          <div className="flex justify-end gap-2">
            <Button
              ...
              onClick={() => navigate(isUserAuth ? "/orders" : "/inquiry")}
            >
              {isUserAuth ? "My Orders" : "Request Quote"}
            </Button>
            <Button
              variant="ghost"
              className="gap-2 text-sm text-foreground"
              onClick={() => navigate(isOwnerAuth ? "/owner" : "/owner/login")}
            >
      ...
      {/* Auth Dialog */}
      <Dialog open={authMode !== "none"} onOpenChange={handleAuthClose}>
        <DialogContent className="sm:max-w-md" showCloseButton={authMode !== "otp"}>
          <div className="flex justify-center">
            {authMode === "login" && (
              <LoginForm
                onSuccess={handleAuthSuccess}
                onSwitchToSignup={() => setAuthMode("signup")}
                onSwitchToPasswordReset={() => setAuthMode("password-reset")}
              />
            )}
            {authMode === "signup" && (
              <SignupForm
