...
export const ADMIN_UID = 'zh5ODSgQOCgkkz74zNulyeS36XL2';
...
export const ensureUserProfileExists = async (userId: string, profileData?: UserProfileData) => {
  try {
    ...
    if (!existing.exists()) {
      ...
      console.log(`Created new user profile for userId: ${userId}`);
    } else {
      await setDoc(profileRef, {
        uid: userId,
        ...profileData,
      ...
      console.log(`Updated existing user profile for userId: ${userId}`);
    }
